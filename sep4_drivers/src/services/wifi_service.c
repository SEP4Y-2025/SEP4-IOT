...
void wifi_service_init_best(const wifi_credential_t *nets, uint8_t nets_len)
{
  ...
  // ── immediately try to connect to the best network (10 s scan timeout)
  if (network_controller_connect_best(known_nets, known_nets_len, /*scan*/ 10))
  {
    state = CONNECTED;
    logger_service_log("Wi-Fi: initial connect OK");
  }
  else
  {
    state = DISCONNECTED;
    logger_service_log("Wi-Fi: initial connect FAILED – will retry in poll()");
  }
}

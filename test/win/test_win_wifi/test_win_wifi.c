...
{
  UART_Callback_t cb = uart_init_fake.arg2_val; // the callback wifi_command() installed
  if (reply_idx < reply_len)
  {
    cb((uint8_t)reply_data[reply_idx++]);
  }
  // no actual delay
}
...
void setUp(void)
{
  ...
  _delay_ms_fake.custom_fake = my_delay_ms;
...
void fake_wifiModule_send(const char *cArray, int length)
{
  reply_data = cArray;
  reply_len = length;
  reply_idx = 0;
